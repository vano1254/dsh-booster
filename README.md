...
| 待补 | ① ✅ `tools/setup-code-server.ps1`：下载 / 校验 / 解压 / 装扩展 / 起服务合成一条命令 ② ✅ 装完问一次：探到服务就用它，探不到就本次改用内置预览并在设置页说明（**不改写用户选的设置**） ③ macOS/Linux 的启动路径——不做就在 README 顶上明写 Windows only |
