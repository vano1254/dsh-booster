...
DSH Web GUI 的增强套件：**模块化、可开关、改完即时生效**。

一个包覆盖外观与交互优化，但没有任何模块是强制的——没开的模块连槽位都不注册、一个定时器都不起、一行 CSS 都不注入。这就是它"全面但不臃肿"的方式。
...
dsh plugin --profile web add https://github.com/<you>/dsh-booster/archive/refs/tags/v0.1.0.tar.gz
...
已经跑通的是"基座 + 三个模块"：能装、能开、能关、能持久化、重启后还在。

| 版本 | 计划 |
|---|---|
| ✅ v0.2 | 侧栏预览（写代码时自动打开文件、给出链接时自动加载网页/视频）、右侧 VS Code 的独立标签页 + 设置页入口 |
| 待补（**分享给别人之前必做**） | ① 一键装机脚本：下载 / 校验 / 解压 / 装扩展 / 起服务合成一条命令 ② 检测不到 code-server 时给出可见提示，并把**新装默认**改为内置预览（现在默认 `vscode`，没装的人会遭遇沉默失败） ③ macOS/Linux 的启动路径——不做就在 README 顶上明写 Windows only |
| v0.3 | 命令面板（Ctrl+K 快速跳转）、消息增强（复制 Markdown / 折叠长消息 / 会话内搜索） |
| v0.4 | 上下文与成本看板（右栏，接宿主 `tokenMeter` 读真实 token 与花费）、输入区增强（草稿暂存、提示词快插） |
| 待定 | 密度/消息宽度（需要先盘清产品暴露的间距 token，避免硬写 DOM 选择器） |

**明确不做**：系统通知、插件市场/管理器、语音、消息编辑/reroll。这些社区已有成熟实现（`dsh-notify-win`、`dshmarket`、`dsh-voice-chat`、`dsh-message-edit`），重复实现只会让插件变臃肿。
